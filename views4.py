...
def login():
    if 'access_token' in session:
        ...
        # แทนที่จะรีไดเรกต์กลับมาที่ /login ให้ไปที่หน้า OAuth consent เพื่อขอสิทธิ์การเข้าถึง
        return redirect('https://oauth.cmu.ac.th/authorize?client_id=26GxenhhK15m5n2prKgjQYF33tT0uMjnwkC0Y2Z7&response_type=code&redirect_uri=http://localhost:5000/callback')
...
def callback():
    ...
    token_response = requests.post('https://oauth.cmu.ac.th/oauth/token', data={
    ...
    if token_response.status_code != 200:
        return f"Error: Unable to fetch token, status code {token_response.status_code}", 500
...
def protected():
    ...
    user_info_response = requests.get('https://api.cmu.ac.th/protected_resource', headers=headers)
    user_info = user_info_response.json()
    
    return f'User Info: {json.dumps(user_info, indent=4)}'
